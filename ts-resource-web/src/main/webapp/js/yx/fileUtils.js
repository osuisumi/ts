function initRemoteUploader(server, fileNumLimit) {
...
//		uploader.upload();
//	});
	$(document).on('click', '#fileList .dlt', function() {
		var _this = $(this);
...
function initFileParam() {
	...
	$('#fileList').find('.fileLi.success').each(function(i) {
		...
		$form.append('<input class="fileParam" name="fileInfos[' + i + '].id" value="' + fileId + '" type="hidden"/>');
		$form.append('<input class="fileParam" name="fileInfos[' + i + '].fileName" value="' + fileName + '" type="hidden"/>');
		$form.append('<input class="fileParam" name="fileInfos[' + i + '].url" value="' + url + '" type="hidden"/>');
	});
}
...
function initVideoUploader(url,callBack){
	var uploader = WebUploader.create({
		// swf文件路径
		swf : $('#ctx').val() + '/js/webuploader/Uploader.swf',
		// 文件接收服务端。
	...
	uploader.on('fileQueued', function(file) {
		$list = $('#videoList');
		$('#videoList').empty();
		$list.append('<li id="' + file.id + '" class="fileLi fileItem item">' + '<a href="#" class="txt">' + file.name + '</a>' + '<div class="state">' + '<span class="status">等待上传...</span>' + '</div>' + '</li>');
		uploader.upload();
	});
	...
	uploader.on('uploadSuccess', function(file, response) {
		if(response.responseCode == '00'){
			...
			if(url.indexOf("Temp")!=-1){
				fileInfo = response.responseData;
			}else{
				fileInfo = response.responseData.newestFile;
			}
			$('#videoPreView').append('<input type="hidden" name="video.id" value="'+fileInfo.id+'" />');
			$('#videoPreView').append('<input type="hidden" name="video.fileName" value="'+fileInfo.fileName+'" />');
			$('#videoPreView').append('<input type="hidden" name="video.url"   value="'+fileInfo.url+'"/>');
			$('#videoPreView').append('<input type="hidden" name="video.remark"   value="'+fileInfo.remark+'"/>');
			$('.status').text('已上传');
			if(callBack){
				callBack();
			}
