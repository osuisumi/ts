...
//上传多张图片并显示缩略图
function initImagesUploader(relationId,type){
    var $ = jQuery,
    $list = $('#fileList'),
...
uploader = WebUploader.create({
    auto: true,
    swf: $('#ctx').val() + '/js/webuploader/Uploader.swf',
    server: $('#ctx').val() + '/file/uploadTemp.do?relationId='+relationId+'&type='+type+'',
    pick: '#filePicker',
    resize : true,
    // 只允许选择文件，可选。
...
uploader.on( 'fileQueued', function( file ) {
    var $li = $(
    		/*
            '<div style="float:left;width:100px;height:140px" id="' + file.id + '" class="file-item thumbnail">' +
                '<img>' +
                '<div class="info">' + file.name + '</div>' +
            '</div>'*/
    		'<li class="fileLi fileItem item" style="display:inline-block" id="'+file.id+'">'+
    		'<img>'+
...
uploader.on( 'uploadError', function( file ) {
    ...
    // 避免重复创建
    if ( !$error.length ) {
        $error = $('<div style="display:block" class="error"></div>').appendTo( $li);
    }
    $error.text('上传失败');
});
...
function initImageParam(form){
	$form = $('#'+form);
	$form.html('');
	$('#fileList').find('.upload-state-done').each(function(i) {
		var fileId = $(this).attr('fileId');
		var fileName = $(this).attr('fileName');
		var url = $(this).attr('url');
		$form.append('<input class="fileParam" name="fileInfos[' + i + '].id" value="' + fileId + '" type="hidden"/>');
